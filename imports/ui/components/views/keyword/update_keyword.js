...
class UpdateKeyword extends React.Component{
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    const {keywordInfo} = nextProps;
    if(keywordInfo) {
      const {name, nature, number} = keywordInfo;
      this.setState({keywordName: name, keywordNum: number, keywordNature: nature})
    }
  }
  ...
  // 更新记录
  handleSubmit(e) {
    const keywordId = this.props.keywordInfo._id;

    Submit(this, 'keywords.update', {keywordId, ...this.state});
  }
  ...
  render() {
    ...
    return(
      <div
        className="modal fade" id="keywordUpdate" tabIndex="-1"
