...
class UpdateRoom extends React.Component{
  ...
  componentWillReceiveProps(props) {
    if(props.roomInfo && Object.keys(props.roomInfo).length > 0) {
      const roomName = props.roomInfo.roomName;
      const roomType = props.roomInfo.roomType;
      const saveTime = props.roomInfo.saveTime;
      this.setState({roomName, roomType, saveTime});
    }
  }
  ...
  // 更新记录
  handleSbumit(e) {
    const roomId = this.props.roomInfo.roomId || '';
    const roomInfo = {roomId, ...this.state};

    Submit(this, 'rooms.update', roomInfo);
  }
  ...
  render() {
    ...
    return(
      <div
        className="modal fade" id="roomUpdate" tabIndex="-1"
